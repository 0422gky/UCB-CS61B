...
public class DebugExercise2 {
     ...
     */
    // the original max() function is buggy and we will rewrite it
    ...
    // this add function is buggy too for the incredible bit ops used in it,
    // reminding me of datalab
    ...
    public static int arraySum(int[] x) {
        ...
        int sum = 0;
        while (i < x.length) {
            // sum = sum + add(sum, x[i]); this line is buggy too for adding sum 2 times
            sum = add(sum, x[i]);
            i = i + 1;
        }
        return sum;
